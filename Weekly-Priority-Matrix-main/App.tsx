...
const App: React.FC = () => {
  ...
  const archiveWeek = () => {
    ...
    // Fix: Explicitly casting state to DayState and Object.entries result to handle 'unknown' property access errors.
    setRows(prevRows => prevRows.map(row => ({
      ...row,
      days: (Object.entries(row.days) as [DayOfWeek, DayState][]).reduce((acc, [day, state]) => {
        acc[day] = state.completed ? { text: '', completed: false } : state;
        return acc;
  ...
  return (
    <div className="flex h-screen bg-[#f8fafc] overflow-hidden">
      ...
      {/* Fix: Added explicit type casting (Object.values as DayState[]) to prevent 'unknown' property access errors in planContext map. */}
      <ChatBot isOpen={isAssistantOpen} onClose={() => setIsAssistantOpen(false)} planContext={(view === 'current' ? rows : (history[historyIndex]?.rows || [])).map(r => `${r.label}: ${(Object.values(r.days) as DayState[]).map(d => d.text).join(' | ')}`).join('\n')} userProfile={{ name: settings.userName, email: settings.companyEmail }} />
    </div>
  );
};
